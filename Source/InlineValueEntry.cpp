...
InlineValueEntry::InlineValueEntry(float currentValue) {
...
void InlineValueEntry::setValueChangeCallback(std::function<void(float)> callback) {
...
void InlineValueEntry::setExitKeyLambdas() {
    editor.onReturnKey = [this] {
        if (valueChangeCallback) {
            valueChangeCallback((float)editor.getText().getDoubleValue());
        }
